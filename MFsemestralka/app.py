...
def convolve(image: np.ndarray, kernel: np.ndarray) -> np.ndarray:
    """
    Perform 2D convolution using NumPy vectorization.
    - Uses efficient striding instead of explicit loops.
    - Applies convolution on the entire image at once.
    """
    kernel_size = kernel.shape[0]
    pad_size = kernel_size // 2  # Padding size for kernel alignment
    padded = np.pad(image, pad_size, mode='reflect')  # Reflective padding

    # Extract patches using NumPy strides for fast convolution
    shape = (
        image.shape[0], image.shape[1], kernel_size, kernel_size
    )
    strides = (
        padded.strides[0], padded.strides[1], padded.strides[0], padded.strides[1]
    )
    patches = np.lib.stride_tricks.as_strided(padded, shape=shape, strides=strides)

    # Apply convolution using NumPy broadcasting
    convolved = np.einsum('ijkl,kl->ij', patches, kernel, optimize=True)
    return np.clip(convolved, 0, 255).astype(np.uint8)


def apply_edge_detection(image: Image.Image, method: str) -> Image.Image:
    """
    Edge detection using optimized NumPy-based convolution.
    - 'sobel': Sobel filter (gradient-based)
    - 'laplacian': Laplacian filter (second derivative)
    """
    ...
    if method == "sobel":
        ...
        edges = np.hypot(edges_x, edges_y)  # Compute gradient magnitude
...
def process_image(image_file, brightness, solarization, negative, negative_type, resize_percent, edge_detection):
    """
...
def upload():
    ...
    negative_type = request.form.get('negative-type', 'color')
